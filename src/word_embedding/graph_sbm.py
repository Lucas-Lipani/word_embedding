...
def sbm(
    ...
    Params
    ------
    layered : usa o modelo em camadas (LayeredBlockState)
    n_blocks : fixa o número de blocos (apenas no modo não-nested)
    nested : usa SBM hierárquico (minimize_nested_blockmodel_dl). Ignora n_blocks.
    """
    if layered:
        state_args = {
            "ec": g.ep["layer"],
        ...
        state_cls = LayeredBlockState
    else:
        state_args = {
            "eweight": g.ep["weight"],
            "pclabel": g.vp["tipo"],
        }
        state_cls = None  # BlockState padrão
